...
int main() {
    try {
        ...
        movie1.addRating(8.0);
        movie2.addRating(8.5);
        movie3.addRating(9.0);
        movie4.addRating(9.2);
        movie5.addRating(8.8);
        movie6.addRating(9.1);
        movie7.addRating(9.5);
        ...
        user1.displayAddedMovies();

        std::cout << "\nFilmes de " << user2.name << ":\n";
        user2.displayAddedMovies();

        std::cout << "\nFilmes de " << user3.name << ":\n";
        user3.displayAddedMovies();
        ...
        // Administra os usuários e filmes
        Admin admin(0, "Admin", "admin@exemplo.com", "admin123");

        // Adicionar um novo usuário
        User newUser(4, "Carla", "carla@exemplo.com", "senha012");
        admin.addUser(users, newUser);

        // Editar um usuário existente
        admin.editUser(users, 1, "Joao Silva", "joao.silva@exemplo.com", "novaSenha123");

        // Deletar um usuário
        admin.deleteUser(users, 2);

        // Adicionar um novo filme
        Movie newMovie(8, "The Lion King", "Animation", 1994, "Roger Allers, Rob Minkoff", 8.5, false);
        admin.addMovie(movies, newMovie);

        // Editar um filme existente
        admin.editMovie(movies, 3, "Interstellar", "Sci-Fi", 2014, "Christopher Nolan", 8.7);

        // Deletar um filme
        admin.deleteMovie(movies, 4);
